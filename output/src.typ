...
#let work-heading(title, company, location, start-date, end-date, body) = {
  ...
  generic_2x2(
    (1fr, 1fr),
    [*#title*], [*#period(start-date, end-date)*],
...
#let skill-heading(name, stack: "", project-url: "", body) = {
...
#let education-heading(institution, location, degree, major, start-date, end-date, body) = {
  ...
  generic_2x2(
    ...
    [#degree #major], period(start-date, end-date)
  )
  v(-0.2em)
  if body != [] {
    v(-0.4em)
...
#let education-extra(title, start-date, end-date, body) = {
  generic_1x2(
    title,
